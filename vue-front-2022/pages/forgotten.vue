<template>
  <div class="main">
    ...
    <div class="title" >
      ...
      <span>·</span>
      <a class="active" href="/forgotten">Forgotten</a>
    </div>
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item
          class="input-prepend"
          prop="password"
          :rules="[{ required: true, message: 'Please enter new Password', trigger: 'blur' }]"
        >
          <div>
            <el-input
              type="password"
  ...
  methods: {
    ...
    getCodeFun() {
      ...
      this.$refs.userForm.validateField("email", (errMsg) => {
        if (errMsg == "") {
          registerApi.sendCode(this.params.mobile,this.params.email).then((res) => {
            this.sending = false;
            this.timeDown();
          });
        }
    ...
    submitReset() {
      this.$refs["userForm"].validate((valid) => {
        if (valid) {
          registerApi.resetPassword(this.params.mobile, this.params.email, this.params.code, this.params.password).then((response) => {
            //提示注册成功
            this.$message({
              type: "success",
